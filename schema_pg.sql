-- schema_pg.sql
-- Schema PostgreSQL para Dashboard Operaciones
-- Equivalente a schema.sql pero con tipos nativos de PostgreSQL.
...
-- Documentos / Movimientos / Auditoría / Adjuntos
...
CREATE INDEX IF NOT EXISTS idx_go_nro_orden ON gestion_operativa (nro_orden);
CREATE INDEX IF NOT EXISTS idx_go_fecha_orden ON gestion_operativa (fecha_orden);
CREATE INDEX IF NOT EXISTS idx_go_nombre ON gestion_operativa (nombre);
CREATE INDEX IF NOT EXISTS idx_go_proyecto ON gestion_operativa (proyecto);
...
CREATE INDEX IF NOT EXISTS idx_re_estado_autorizacion ON remitos_entrega (estado_autorizacion);
